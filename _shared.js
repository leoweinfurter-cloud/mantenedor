      ...
      f.planos=f.planos.map(function(p){
        ...
        pe.acoes.forEach(function(a){
          if(a.tipo==="medicao"&&a.valor_exec!=null&&a.valor_exec!==""){
            ...
            f.preditiva.push({id:nextId(),data:hoje,desc:a.desc||"",param:a.param||"",unidade:a.unidade||"",valor:valor,limite:parseFloat(a.limite_max)||0,limite_min:parseFloat(a.limite_min)||0,limite_max:parseFloat(a.limite_max)||0,obs:a.obs_exec||""});
...
function initApp(renderFn){
  ...
  Promise.all([
    ...
    if(Array.isArray(dbAt)&&dbAt.length){
      ...
      (dbMed||[]).forEach(function(m){if(fichas[m.ativo_id])fichas[m.ativo_id].preditiva.push({id:m.id,data:m.data,desc:m.desc,param:m.param,unidade:m.unidade,valor:m.valor,limite:m.limite,limite_min:m.limite_min,limite_max:m.limite_max,obs:m.obs});});
